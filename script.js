...
// ===== PREVENT SCROLL WHEN MOBILE NAVBAR IS OPEN =====
document
...
function formatFacilityOption(option) {
  ...
  return $(
    '<span><i class="fas fa-hospital-user me-2"></i>' + option.text + "</span>"
  );
}
...
document.getElementById("staffHours")?.addEventListener("input", function () {
  ...
  document.getElementById("hoursValue").textContent =
    "UGX " + monthlySavings.toLocaleString() + "/mo";
...
document.getElementById("noShowRate")?.addEventListener("input", function () {
  ...
  document.getElementById("noShowValue").textContent =
    "UGX " + monthlyLoss.toLocaleString() + "/mo loss";

  updateROITotal();
});

function updateROITotal() {
  const staffHours = parseInt(
    document.getElementById("staffHours")?.value || 60
  );
  const noShowRate = parseInt(
    document.getElementById("noShowRate")?.value || 30
  );
  ...
  document.getElementById("timeSavingsCalc").textContent =
    Math.round(staffHours * 0.6) + " hours";
  document.getElementById("timeSavingsValue").textContent =
    "UGX " + Math.round(timeSavings).toLocaleString();

  document.getElementById("noShowReductionCalc").textContent =
    Math.round(noShowRate * 0.2) + "%";
  document.getElementById("noShowSavingsValue").textContent =
    "UGX " + Math.round(noShowReduction).toLocaleString();

  document.getElementById("totalBenefitCalc").textContent =
    "UGX " + Math.round(totalBenefit).toLocaleString();
...
function nextStep(step) {
  ...
  // Scroll to form
  document
    .querySelector(".multi-step-form-card")
...
document
  .getElementById("multiStepDemoForm")
  ...
  if (button.textContent.includes("Watch")) {
    button.addEventListener("click", function () {
      trackEvent("video_played", {
        video_title:
          this.closest(".video-card")?.querySelector("h5")?.textContent,
      });
    });
  }
});
...
document.querySelectorAll(".kb-link").forEach((link) => {
  link.addEventListener("click", function (e) {
    const guideTitle =
      this.closest(".kb-card")?.querySelector("h5")?.textContent;
...
document.addEventListener("DOMContentLoaded", () => {
  // Reveal cards, sections, and important elements
  const revealElements = document.querySelectorAll(
    ".step-card, .feature-card-premium, .card, .section-light, .section-dark, " +
      ".metric-card, .benefit-item, .savings-metric, .testimonial-card, .team-card, " +
      ".logo-card, .pricing-card-premium, .stat-card, .care-feature, .kb-card, " +
      ".video-card, .contact-method, .contact-method-enhanced, .form-card-enhanced, " +
      "[data-reveal]"
  );
...
console.log(
  "All features active: Forms, Navigation, Animations, Select2, ROI Calculator, Multi-step Forms, Webinar Scheduler, Mobile Analytics, Dark Mode, Video Demos, Knowledge Base, Team Profiles, Customer Logos, Analytics Tracking, Scroll Reveal Animations"
);
